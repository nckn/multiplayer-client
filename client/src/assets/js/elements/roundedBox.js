...
export default function RoundedBoxGeometry(
  ...
  function doVertices() {
    ...
    for (var i = 1; i < 8; i++) {
      ...
      for (var jj = 0; jj < cornerVerts[0].length; jj++) {

        const vert = cornerVerts[0][jj].clone().multiply(cornerLayout[i]);

        cornerVerts[i].push(vert);

        vertexPool.push(vert);

        const norm = cornerNormals[0][jj].clone().multiply(cornerLayout[i]);
