  ...
  .container.outer.black-bg(ref="master_cont")
    ...
    .container.inner.overlay
      ...
      .toggle-wrapper
        ...
        label.switch(for="switch") Toggle
      //- Color picker
      #picker
      //- Debug
      //- .debug-info.hide-it
...
var randomColors = [
  ...
  0xff3a33, // salmon
  0xffb012, // Yellow
  0x00ff1a, // Green bright
  0xf419ff, // Dark purble
  0x28ff03, // Yellowish green
  0xff2b5d, // Burgundy
  0x2962ff, // Light blue
]
...
export default {
  ...
  async mounted () {
    ...
    setTimeout(() => {
      ...
      // self.makeBlackCenterCube()

      self.setupGridPlanes()
  ...
  methods: {
    ...
    makeCube() {
      ...
      var randomColor = randomColors[Math.floor(Math.random() * Math.floor(randomColors.length))]
      ...
      for (var i = 0; i < self.noOC; i++ ) {
        ...
        for (var x = 0; x < grid.x; x++) {
          for (var y = 0; y < grid.y; y++) {
            var cube = self.makeCube()
            cube.position.x = x
            cube.position.y = y
            groupsubGroup.add( cube )
          }
        }

        groupMesh.add( groupsubGroup )
